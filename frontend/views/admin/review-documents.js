...
async function loadOrganizationData() {
  try {
    const response = await fetch(
      `/Org-Accreditation-System/backend/api/organization_api.php?org_id=${orgId}`,
      {
        method: "GET",
      }
    );
    ...
    if (result.status === "success" && result.data) {
      ...
      document.title = `Review Documents - ${
        result.data.org_name || "Organization"
      }`;
    }
  } catch (error) {
...
async function loadDocuments() {
  ...
  try {
    const response = await fetch(
      `/Org-Accreditation-System/backend/api/document_api.php?org_id=${orgId}`,
      {
        method: "GET",
      }
    );
...
function applyFiltersAndRender() {
  ...
  filteredDocuments = allDocuments.filter((doc) => {
    // Search filter
    if (searchQuery) {
      const searchableText = `${doc.requirement_name} ${doc.file_name} ${
        doc.remarks || ""
      }`.toLowerCase();
  ...
  filteredDocuments.sort((a, b) => {
    switch (sortBy) {
      ...
      case "requirement":
        return (a.requirement_name || "").localeCompare(
          b.requirement_name || ""
        );
      case "status":
...
function renderDocuments() {
  ...
  const totalPages =
    itemsPerPage === Infinity ? 1 : Math.ceil(totalItems / itemsPerPage);
    ...
    metadata.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <span class="text-gray-600">${escapeHtml(
                      doc.requirement_name || "N/A"
                    )} (${escapeHtml(doc.requirement_type || "N/A")})</span>
                </div>
                <div>
...
async function updateStatus(documentId, status, remarks = null) {
  ...
  try {
    ...
    const response = await fetch(
      "/Org-Accreditation-System/backend/api/document_api.php",
      {
        method: "PUT",
        headers: {
    ...
    if (result.status === "success") {
      if (status === "verified") {
        ...
        showSuccess(
          "Document returned for revision. The organization will be notified."
        );
      } else {
